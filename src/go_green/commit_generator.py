...
class CommitGenerator:
    ...
    def __init__(
        ...
        self.selected_weekend_days = set()  # Weekend days selected for commits in current week
    ...
    def _select_weekdays_for_week(self, week_start: datetime) -> set:
        ...
        """
        # Select 2-3 weekdays randomly for a realistic work pattern
        # Lower weights favor fewer weekdays
        if self.weekday_weight < 0.3:
            # Low weight: 2-3 weekdays
    ...
    def _select_weekend_days_for_week(self, week_start: datetime) -> set:
        """
        Select which weekend days (Sat-Sun) should have commits for a given week.
        Selects 1-2 weekend days based on weekend_weight.

        Args:
            week_start: Monday of the week

        Returns:
            Set of weekend day numbers (5=Saturday, 6=Sunday) that should have commits
        """
        # Determine how many weekend days based on weekend_weight
        if self.weekend_weight < 1.0:
            # Low weight: 0-1 weekend days
            if random.random() > self.weekend_weight:
                return set()
            num_weekend_days = 1
        else:
            # Higher weight: 1-2 weekend days
            num_weekend_days = random.randint(1, 2)

        # Randomly select weekend days (5=Saturday, 6=Sunday)
        weekend_days = [5, 6]
        return set(random.sample(weekend_days, num_weekend_days))
    ...
    def _calculate_commit_count(self, date: datetime) -> int:
        ...
        # Check if we're in a new week and need to select new weekdays and weekend days
        week_start = self._get_week_start(date)
        if week_start != self.current_week_start:
            ...
            self.selected_weekend_days = self._select_weekend_days_for_week(week_start)
        ...
        if self._is_holiday(date):
            ...
            # Weekends: only selected weekend days get commits
            if date.weekday() not in self.selected_weekend_days:
                return 0
            # Selected weekend days can have increased activity
    ...
    def get_weekly_pattern(
        ...
        """
        Analyze commits and generate weekly pattern summary.

        Args:
            commits: List of commit datetimes
            start_date: Optional start date to include all weeks in range
            end_date: Optional end date to include all weeks in range

        Returns:
            List of tuples (week_commits, is_vacation) where week_commits is a dict
            with day-of-week (0-6) as keys and commit counts as values, and is_vacation
            is True for vacation weeks
        """
        # Group commits by week and day
        weekly_commits = defaultdict(lambda: defaultdict(int))

        for commit_dt in commits:
            # Get the Monday of the week for this commit (date only, no time)
            week_start = self._get_week_start(commit_dt).date()
            # Get day of week (0=Monday, 6=Sunday)
            day_of_week = commit_dt.weekday()
            weekly_commits[week_start][day_of_week] += 1

        # If start_date and end_date are provided, include all weeks
        if start_date and end_date:
            result = []
            current_week = self._get_week_start(start_date).date()
            end_week = self._get_week_start(end_date).date()

            while current_week <= end_week:
                # Check if this week is a vacation week
                is_vacation = any(
                    vw.date() == current_week for vw in self.vacation_weeks
                )

                # Get commits for this week (empty dict if none)
                week_data = dict(weekly_commits.get(current_week, {}))
                result.append((week_data, is_vacation))

                current_week += timedelta(days=7)

            return result
        else:
            # Legacy behavior: only return weeks with commits
            result = []
            for week_start in sorted(weekly_commits.keys()):
                result.append((dict(weekly_commits[week_start]), False))
            return result
