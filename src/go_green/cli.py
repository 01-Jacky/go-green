...
def main(
    ...
    try:
        ...
        if dry_run or commits:
            if dry_run:
                # Show weekly pattern preview for dry run
                console.print("[bold cyan]Commit Pattern Preview:[/bold cyan]\n")

                # Parse dates to pass to get_weekly_pattern
                parsed_start = parser.parse(start_date)
                parsed_end = parser.parse(end_date)
                weekly_pattern = generator.get_weekly_pattern(
                    commits, start_date=parsed_start, end_date=parsed_end
                )
                ...
                for week_num, (week_data, is_vacation) in enumerate(weekly_pattern, 1):
                    if is_vacation:
                        console.print(f"  week {week_num}: [yellow]VACATION[/yellow]")
                    else:
                        ...
                        for day in range(7):  # 0=Monday through 6=Sunday
                            if day in week_data:
                                count = week_data[day]
                                week_parts.append(f"{day_abbrev[day]}{count}")

                        if week_parts:
                            week_str = " ".join(week_parts)
                            console.print(f"  week {week_num}: {week_str}")

                console.print()
            else:
                # Show sample of commits for live mode
